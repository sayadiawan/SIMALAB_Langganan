    ...
    <table width="100%" cellspacing="0" cellpadding="0">
        <tr>
            <td style="text-align: center">
                <h3><strong>PEMERIKSAAN {{ $item_program->name_program }}</strong></h3> <br>
                <h3><strong>Dari Tanggal {{ fdate_sas($from_tanggal, 'DDMMYYYY') }} Sampai Tanggal
                        {{ fdate_sas($to_tanggal, 'DDMMYYYY') }}</strong></h3>
            </td>
        </tr>
    </table>
    ...
    <table width="100%" cellspacing="0" cellpadding="0" border="1" style="margin-top: 5px">
        <thead>
            <tr>
                ...
                <th style="width: 30%" colspan="{{ count($data_sample_method->where('sample')) }}">HASIL PEMERIKSAAN
                    {{ strtoupper($item_program->name_program) }}</th>
            </tr>

            <tr>
                @foreach ($data_sample_method->where('sample') as $item)
                    <th>{{ $item->params_method }}</th>
                @endforeach
            </tr>
        </thead>
        ...
        <tbody>
            ...
            @if (count($data_sample) > 0)
                @foreach ($data_sample as $key_ds => $item_ds)
                    <tr>
                        <td>{{ $key_ds + 1 }}</td>
                        <td>{{ \Carbon\Carbon::createFromFormat('D MMMM Y HH:mm', $item_ds->date_penanganan_sample) }}
                        </td>
                        <td>{{ $item_ds->location_samples ?? '-' }}</td>
                        <td>{{ $item_ds->sampletype->name_sample_type }}</td>
                    </tr>
                @endforeach
            @else
                <tr>
