...
<body style="margin: 10px; padding:0">
    ...
    <table width="100%" cellspacing="0" cellpadding="0">
      ...
      <td>
        ...
        {{ $lab_string }}
        @else
        - /Rektal Swab/{{ getRomawi(date('m')) }}/{{ date('Y') }}
        @endif
      </td>
    </tr> --}}
            ...
            <td style="vertical-align: text-top;">
                @php
                    ...
                    foreach ($method_all as $method) {
                        $parameterList .= $prefix . '<i>' . $method->name_report . '</i>';
                        $prefix = ', ';
                    }
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <td style="text-align: center; width: 15%" rowspan="2">Nama</td>
            <td style="text-align: center; width: 10%" rowspan="2">L/P</td>
            <td style="text-align: center; width: 10%" rowspan="2">Umur th</td>
            <td style="text-align: center; width: 40%" colspan="{{ count($method_all) }}">Hasil Pemeriksaan</td>
            <td style="text-align: center; width: 15%" rowspan="2">Batas Syarat</td>
        </tr>

        <tr>
            @foreach ($method_all as $method)
                <td style="text-align: center; width: 10%"><i>{{ $method->name_report }}</i></td>
            @endforeach
        </tr>
        ...
        @foreach ($table as $mytable)
            <tr>
                ...
                <td style="text-align: left">{!! $mytable['sample_type']->location_samples !!}
                </td>

                <td style="text-align: center">{{ $mytable['sample_type']->gender_samples }}</td>

                <td style="text-align: center">{{ $mytable['sample_type']->umur_samples }}</td>
