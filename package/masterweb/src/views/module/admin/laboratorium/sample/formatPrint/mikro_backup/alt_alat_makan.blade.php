...
<body style="margin: 10px; padding:0">
    ...
    <table width="100%" cellspacing="0" cellpadding="0">
      ...
      <td>
        ...
        {{ $lab_string }}
        @else
        - /Peralatan Makan-BAK/{{ getRomawi(date('m')) }}/{{ date('Y') }}
        @endif
      </td>
    </tr> --}}
        ...
        @foreach ($table as $mytable)
            <tr>
                ...
                <td style="text-align: left">{!! $mytable['sample_type']->location_samples !!}
                    {{-- /
        {{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $mytable['sample_type']->datesampling_samples)->format(
        'd-m-Y',
        ) }} --}}
                </td>
                ...
                <td rowspan="{{ count($table) }}" style="text-align: center">
                    @if (count($mytable['result']) <= 1)
                        @foreach ($mytable['result'] as $result)
                            {!! $result['nilai_baku_mutu'] !!}
                        @endforeach
                    @else
                        @php
                            $i = 0;
                        @endphp
                        @foreach ($mytable['result'] as $result)
                            {{ $method_all[$i]->name_report }}:
                            {!! $result['nilai_baku_mutu'] !!}<br><br>
                            @php
                                $i++;
                            @endphp
                        @endforeach
