    ...
    <tbody>
        <tr id="registrasi">
            ...
            @else
                <form action="{{ route('elits-samples.verification-analytic-2', [$sample->id_samples]) }}"
                    ...
                    <td><input type="datetime-local" class="form-control datetime" name="start_date" required>
                    </td>
                    <td><input type="datetime-local" class="form-control datetime" name="stop_date" required>
                    </td>
                    <td>
        ...
        <tr id="analitik">
            ...
            @if (isset($listVerifications[2]))
                <td>{{ \Smt\Masterweb\Helpers\DateHelper::formatDate($listVerifications[2]->start_date) }}</td>
                <td>{{ \Smt\Masterweb\Helpers\DateHelper::formatDate($listVerifications[2]->stop_date) }}</td>
        ...
        <tr id="baca-hasil">
            ...
            @if (isset($listVerifications[3]))
                <td>{{ \Smt\Masterweb\Helpers\DateHelper::formatDate($listVerifications[3]->start_date) }}</td>
                <td>{{ \Smt\Masterweb\Helpers\DateHelper::formatDate($listVerifications[3]->stop_date) }}</td>
        ...
        <tr id="verifikasi">
            ...
            @if (isset($listVerifications[4]))
                <td>{{ \Smt\Masterweb\Helpers\DateHelper::formatDate($listVerifications[4]->start_date) }}</td>
                <td>{{ \Smt\Masterweb\Helpers\DateHelper::formatDate($listVerifications[4]->stop_date) }}</td>
        ...
        <tr id="validasi">
            ...
            @if (isset($listVerifications[5]))
                <td>{{ \Smt\Masterweb\Helpers\DateHelper::formatDate($listVerifications[5]->start_date) }}</td>
                <td>{{ \Smt\Masterweb\Helpers\DateHelper::formatDate($listVerifications[5]->stop_date) }}</td>
        ...
        <tr id="validasi-update" style="display: none;">
            ...
            <form action="{{ route('elits-samples.verification-analytic-2', [$sample->id_samples]) }}"
                ...
                <td class="text-center">
                    <button type="submit" class="btn btn-success"
                        @if (!isset($listVerifications[4]) or !isset($pengesahanHasil) or !isset($pengesahanHasil->pengesahan_hasil_date)) disabled @endif onclick="checkNikAndPassword(document.getElementById('namaPetugasValidasiUpdate').value, 'formValidasiUpdate')">Verifikasi</button>
            </form>
            <a href="{{ route('elits-pengesahan-hasil.index', [$sample->id_samples, $lab_num->lab_id]) }}"><button
                    type="button" class="btn btn-primary"
                    @if (!isset($listVerifications[4])) disabled @endif>Validasi
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // 2. Pemeriksaan (Start dari Stop Pendaftaran)
        @if (isset($listVerifications[2]))
