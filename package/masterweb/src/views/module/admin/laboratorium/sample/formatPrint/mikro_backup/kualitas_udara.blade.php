...
<body style="margin: 10px; padding:0">
    <table width="100%" cellspacing="0" cellpadding="0">
        <tr>
            <td>
                <br>
                <img src="{{ asset('assets/admin/images/logo/kop_perusahaan.png') }}" width="730px">
            </td>
        </tr>
    </table>
    ...
    <table width="100%" cellspacing="0" cellpadding="0">
      ...
      <td>
        ...
        {{ $lab_string }}
        @else
        - /Kualitas Udara/{{ getRomawi(date('m')) }}/{{ date('Y') }}
        @endif
      </td>
    </tr> --}}
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0">
        ...
        @endphp
        @foreach ($table as $mytable)
            @if (count($mytable['result']) > 0)
                <tr style="page-break-after:auto;">
                    <td style="text-align: center; width: 10%" rowspan="2">No Sampel</td>
                    <td style="text-align: center; width: 35%" rowspan="2">Jenis Sampel / Lokasi</td>
                    <td style="text-align: center; width: 20%">Hasil Pemeriksaan</td>
                    @if ($i == 0)
                        <td style="text-align: center; width: 20%" rowspan="2">Batas Syarat</td>
                        <td style="text-align: center; width: 15%" rowspan="2">Satuan</td>
                    @endif
                </tr>

                <tr>

                    <td style="text-align: center; width: 10%">{{ $mytable['method']->name_report }}</td>
                ...
                @foreach ($mytable['result'] as $sample)
                    <tr>
                        <td style="text-align: center">{{ explode('/', $sample['sample']->codesample_samples)[2] }}
                        </td>
                        <td style="text-align: left">{!! $sample['sample']->location_samples !!}
                        </td>

                        {{-- @foreach ($mytable['result'] as $result) --}}
                        <td style="text-align: center">
                            {!! cek_hasil_color(
                                isset($sample['hasil']) ? $sample['hasil'] : (isset($sample['equal']) ? $sample['equal'] : ''),
                                isset($sample['min']) ? $sample['min'] : '',
                                $sample['max'],
                                $sample['equal'],
                                'result_output_method_' . $sample['method_id'],
                                $sample['offset_baku_mutu'],
                            ) !!}
                        </td>
                        {{-- @endforeach --}}
                        ...
                        <td rowspan="{{ count($mytable['result']) }}" style="text-align: center">
                            {{-- ALT
            <ul>
              <li>Ruang Operasi Kosong 35</li>
              <li>Ruang Operasi Dengan Aktifitas 180</li>
              <li>Ruang Operasi Steril 10</li>
              <li>Perinatal/Perawatan 200-500</li>
              <li>ICU Angka 200</li>
            </ul> --}}
                            {!! $sample['nilai_baku_mutu'] !!}

                        </td>

                        <td rowspan="{{ count($mytable['result']) }}" style="text-align: center">
                            {!! $sample['satuan_bakumutu'] !!}</td>
