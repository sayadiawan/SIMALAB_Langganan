...
@section('content')
        ...
        <div class="card">
            ...
            <div class="template-demo">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><span>Tarif Management</span></li>
                </ol>
                </nav>
            </div>
            </div>
    ...
    <div class="card-body">
      <div class="d-flex">
        ...
        <div class="p-2">
            <a href="{{route('elits-rates.create')}}">
            ...
            <table id="order-listing" class="table">
              ...
              <tbody>
                ...
                @foreach ($rates as $rate)
                  @php
                    $major = \Smt\Masterweb\Models\Majors::find($rate->major_rate);
                  @endphp
                <tr>
                    <td>{{$no++}}</td>
                    <td>{{$rate->params_rate}}</td>
                    <td>{{$major->name_major}}</td>
                    <td>Rp. {{$rate->rate_rate}}</td>
                    <td>{{$rate->purpose_rate}}</td>
                    <td>
                       
                      <a href="{{route('elits-rates.edit', [$rate->id_rate])}}">
                        ...
                        <form onsubmit="return confirm('Delete this user permanently?')" 
                            class="d-inline" 
                            action="{{route('elits-rates.destroy', [$rate->id_rate])}}" 
                            method="POST">
