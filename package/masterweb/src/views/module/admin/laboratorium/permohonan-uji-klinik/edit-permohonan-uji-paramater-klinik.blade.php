...
@section('title')
  Ubah Parameter Permohonan Uji Klinik
@endsection
...
@section('content')
  ...
  <div class="card">
    <div class="card-header">
      <h4>Ubah Parameter Permohonan Uji Paket Klinik
      </h4>
              ...
              <tbody>
                    ...
                    <tr id="row_{{ $x }}" class="tr_row">
                      <td style="width: 5%">
                        ...
                        <input type="hidden" class="form-control"
                          name="permohonan_uji_paket_klinik[{{ $value->id_permohonan_uji_paket_klinik }}]"
                          id="permohonan_uji_paket_klinik_{{ $x }}"
                          value="{{ $value->id_permohonan_uji_paket_klinik }}" readonly>
                      </td>

                      <td style="width: 15%">
                        <select class="form-control jenis_parameter" name="jenis_parameter[{{ $x }}]"
                          id="jenis_parameter_{{ $x }}"
                          onchange="setDataJenisParameter({{ $x }})">
                      ...
                      <td style="width: 15%">
                        <select class="form-control type_parameter" name="type_parameter[{{ $x }}]"
                          ...
                          <option value="P" {!! $value->type_permohonan_uji_paket_klinik == 'P' ? 'selected' : '' !!}>Paket</option>
                          <option value="C" {!! $value->type_permohonan_uji_paket_klinik == 'C' ? 'selected' : '' !!}>Custom</option>
                        </select>
                      </td>

                      <td style="width: 35%">
                        {{-- @php
                            dd($value->parameterpaketklinik->name_parameter_paket_klinik);
                          @endphp --}}
                        ...
                        @if ($value->type_permohonan_uji_paket_klinik == 'P')
                          <select class="form-control satuan_parameter" name="satuan_parameter[{{ $x }}][]"
                            ...
                            <option value="{{ $value->parameter_paket_klinik }}" selected>
                        ...
                        @elseif ($value->type_permohonan_uji_paket_klinik == 'C')
                          ...
                          <select class="form-control satuan_parameter" name="satuan_parameter[{{ $x }}][]"
                            id="satuan_parameter_{{ $x }}"
                            onchange="setDataSatuanParameter({{ $x }})" multiple>

                            @foreach ($item_custom_satuan_parameter as $item_csp)
                              <option value="{{ $item_csp->parameter_satuan_klinik }}" selected>
                                {{ $item_csp->parametersatuanklinik->name_parameter_satuan_klinik }}
                              </option>
                            @endforeach
                      ...
                      <td style="width: 15%">
                        <input type="number" class="form-control harga_parameter"
                          ...
                          value="{{ $value->harga_permohonan_uji_paket_klinik }}" readonly>
                      </td>
                ...
                @else
                  <tr id="row_1" class="tr_row">
                    ...
                    <td style="width: 35%">
                      <select class="form-control satuan_parameter" name="satuan_parameter[1][]"
                        id="satuan_parameter_1" onchange="setDataSatuanParameter(1)">
                        <option value=""></option>
                      </select>
                    </td>
                    ...
                    <td style="width: 15%">
                      ...
                      <button type="button" class="btn btn-danger btn-remove-row" data-row="1"
                        onclick="removeRow(1)">
                        <i class="fas fa-minus"></i>
                      </button>
                    </td>
              ...
              <tr>
                ...
                <td>
                  <input type="text" class="form-control" name="subamount_harga_parameter"
                    id="subamount_harga_parameter" value="{{ $item->total_harga_permohonan_uji_klinik }}" readonly>
                </td>
              </tr>
            </table>
          </div>
...
@section('scripts')
  ...
  <script>
    $(document).ready(function() {
      $(".satuan_parameter").select2({
        placeholder: 'Pilih parameter',
        theme: 'classic',
        allowClear: true
      });

      /* setDataJenisParameter();
      setDataPaketParameter();
      setDataSatuanParameter();
      getSubAmount(); */
    });
    ...
    function setDataSatuanParameter(row) {
      ...
      console.log(val_jenis_parameter);
